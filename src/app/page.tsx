import React from 'react';
import { useQuery } from '@tanstack/react-query';
import axios from 'axios';
import MultiSelect from './_components/MultiSelect';
import Link from 'next/link';

const Home = () => {

  return (
    <div className='min-h-screen w-screen flex  flex-col items-center mt-16'>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <div className='max-w-xl mx-auto text-center'>
      <h1 className='mb-4 font-bold'>
        MultiSelect Component
      </h1>
      <p className='text-center mb-4 text-xs'>
        This is a simple MultiSelect component built with React, TypeScript and TailwindCSS. It fetches data from an API and allows you to select multiple items from a dropdown list.
        There may be overflow issues depending on device width.
      </p>
      </div>
      <img src="comp.png" alt="" height={200} width={150} className='hidden md:block' />

      <div className='w-5/6'>
      <MultiSelect />
      </div>
    {/*social*/}
    <div className='mt-8'>
      <Link
        href={"/explanation"}
        className='border p-4 rounded'
      >
        Explanation
      </Link>
    </div>
    <div className='mt-8 flex gap-4'>
      <Link href={"https://github.com/desneck/multiselect"} target='_blank'>
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-github"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg>
      </Link>
      <Link href={"https://www.linkedin.com/in/berat-berk-%C3%A7al%C4%B1k%C4%B1ran/"} target='_blank'>
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-linkedin"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>      </Link>
    </div>
   
    </div>
  );
};

export default Home;
